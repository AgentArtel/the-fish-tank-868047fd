    ...
    <div className="p-6 md:p-8 max-w-3xl space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-lg border bg-card p-4">
          <div className="text-xs text-muted-foreground uppercase tracking-wide">Lifetime spend</div>
          <div className="text-2xl font-semibold tabular-nums mt-1">{money(data.lifetimeSpendCents)}</div>
        </div>
        <div className="rounded-lg border bg-card p-4">
              ...
              <div key={h.id} className="flex items-center gap-3 p-3 text-sm">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">
                    ...
                    {h.itemType ? ` · ${ITEM_TYPE_LABELS[h.itemType as ItemType] ?? h.itemType}` : ""}
                  </div>
                </div>
                {h.kind !== "sale" && (
