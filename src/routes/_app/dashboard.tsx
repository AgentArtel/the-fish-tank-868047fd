...
const getShopOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [recentIntake, recentInventory, upcomingContent, needsMedia, topValue] =
      await Promise.all([
        supabase
          .from("vendor_batches")
        ...
        supabase
          ...
          .select(
            "id, clean_item_name, availability_status, retail_price, created_at, location_id, store_locations(name)",
          )
          .order("created_at", { ascending: false })
          .limit(6),
        supabase
          ...
          .limit(5),
        supabase.from("content_items").select("id, title").eq("status", "needs_media").limit(5),
        supabase
          .from("inventory_items")
...
function Dashboard() {
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      <div
        className="rounded-xl border p-5 flex items-center gap-4 overflow-hidden"
        style={{ background: "var(--brand-ink)", color: "white" }}
      >
        <img
          src="/brand/logo-fish-white.png"
          alt=""
              ...
              {data!.recentIntake.map((b: any) => (
                <li key={b.id} className="p-3 text-sm">
                  <Link
                    to="/batches/$id"
                    params={{ id: b.id }}
                    className="block hover:bg-muted/40 -m-3 p-3 rounded"
                  >
                    <div className="flex justify-between gap-2">
                      ...
                      <span className="text-xs text-muted-foreground shrink-0">
                        {intakeStatusLabel(b.intake_status)}
                      </span>
                    </div>
        ...
        <div className="rounded-xl border bg-card">
          ...
          {(data?.recentInventory ?? []).length === 0 ? (
            <div className="p-4 text-sm text-muted-foreground">
              Nothing yet — convert an intake batch.
            </div>
          ) : (
            <ul className="divide-y">
                ...
                <li key={i.id} className="p-3 text-sm">
                  ...
                  >
                    <div className="flex justify-between gap-2">
                      ...
                      <span className="text-xs text-muted-foreground shrink-0">
                        {i.retail_price != null ? fmtMoney(i.retail_price) : ""}
                      </span>
                    </div>
                    <div className="text-xs text-muted-foreground mt-0.5 flex justify-between">
