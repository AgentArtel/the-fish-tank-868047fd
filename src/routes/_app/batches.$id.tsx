...
import {
  VENDOR_BATCH_SOURCE_TYPES,
  VENDOR_BATCH_SOURCE_LABELS,
  VENDOR_BATCH_INTAKE_STATUSES,
  VENDOR_BATCH_INTAKE_LABELS,
  VENDOR_BATCH_EXTRACTION_STATUSES,
  VENDOR_BATCH_EXTRACTION_LABELS,
  VENDOR_LINE_REVIEW,
  VENDOR_LINE_REVIEW_LABELS,
  VENDOR_LINE_PRICING_LABELS,
  VENDOR_CHARGE_TYPES,
  VENDOR_CHARGE_LABELS,
  ITEM_TYPES,
  ITEM_TYPE_LABELS,
  LOSS_REASONS,
  LOSS_REASON_LABELS,
...
import {
  convertLineItemsToInventory,
  getSignedVendorInvoiceUrl,
  extractBatchWithAI,
  receiveBatchLines,
  uploadDoaPhoto,
  getSignedInventoryMediaUrl,
  promoteQuickAddBatchVendor,
  computeQuickAddReconciliation,
    ...
    <div className="rounded-lg border bg-card p-5 space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <Field label="Document type">
          <Select
            value={f.source_document_type}
            onValueChange={(v) => setF({ ...f, source_document_type: v })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {VENDOR_BATCH_SOURCE_TYPES.map((t) => (
                <SelectItem key={t} value={t}>
                  {VENDOR_BATCH_SOURCE_LABELS[t]}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <Field label="Intake status">
          <Select value={f.intake_status} onValueChange={(v) => setF({ ...f, intake_status: v })}>
            ...
            <SelectContent>
              {VENDOR_BATCH_INTAKE_STATUSES.map((t) => (
                <SelectItem key={t} value={t}>
                  {VENDOR_BATCH_INTAKE_LABELS[t]}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <Field label="Extraction status">
          <Select
            value={f.extraction_status}
            onValueChange={(v) => setF({ ...f, extraction_status: v })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {VENDOR_BATCH_EXTRACTION_STATUSES.map((t) => (
                <SelectItem key={t} value={t}>
                  {VENDOR_BATCH_EXTRACTION_LABELS[t]}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <Field label="Sales order #">
          <Input
            value={f.sales_order_number ?? ""}
            onChange={(e) => setF({ ...f, sales_order_number: e.target.value })}
          />
        </Field>
        <Field label="Customer #">
          <Input
            value={f.customer_number ?? ""}
            onChange={(e) => setF({ ...f, customer_number: e.target.value })}
          />
        </Field>
        <Field label="Carrier">
          <Input
        ...
        <Field label="AWB #">
          <Input
            value={f.awb_number ?? ""}
            onChange={(e) => setF({ ...f, awb_number: e.target.value })}
          />
        </Field>
        <Field label="Terms">
          <Input value={f.terms ?? ""} onChange={(e) => setF({ ...f, terms: e.target.value })} />
        </Field>
        <Field label="Invoice date">
          <Input
            type="date"
            value={f.invoice_date ?? ""}
            onChange={(e) => setF({ ...f, invoice_date: e.target.value || null })}
          />
        </Field>
        <Field label="Ship date">
          <Input
            type="date"
            value={f.ship_date ?? ""}
            onChange={(e) => setF({ ...f, ship_date: e.target.value || null })}
          />
        </Field>
        <Field label="Arrival date">
          <Input
            ...
            onChange={(e) => setF({ ...f, arrival_date: e.target.value || null })}
          />
        </Field>
        <Field label="Subtotal">
          <Input
            type="number"
            step="0.01"
            value={f.invoice_subtotal ?? ""}
            onChange={(e) =>
              setF({
                ...f,
                invoice_subtotal: e.target.value === "" ? null : Number(e.target.value),
              })
            }
          />
        </Field>
        <Field label="Discount">
          <Input
            type="number"
            step="0.01"
            value={f.invoice_discount ?? ""}
            onChange={(e) =>
              setF({
                ...f,
                invoice_discount: e.target.value === "" ? null : Number(e.target.value),
              })
            }
          />
        </Field>
        <Field label="Invoice total">
          <Input
            type="number"
            step="0.01"
            value={f.invoice_total ?? ""}
            onChange={(e) =>
              setF({ ...f, invoice_total: e.target.value === "" ? null : Number(e.target.value) })
            }
          />
        </Field>
        <Field label="Balance due">
          <Input
            ...
            value={f.balance_due ?? ""}
            onChange={(e) =>
              setF({ ...f, balance_due: e.target.value === "" ? null : Number(e.target.value) })
            }
          />
        </Field>
      </div>
        ...
        <label className="inline-flex items-center gap-2 text-sm cursor-pointer">
          ...
          <Button asChild variant="outline" disabled={uploadBusy}>
            <span>
              ...
              {uploadBusy ? "Uploading…" : batch.pdf_storage_path ? "Replace PDF" : "Upload PDF"}
            </span>
          </Button>
        </label>
        {batch.pdf_storage_path && (
          <Button variant="ghost" onClick={openPdf}>
            Open {batch.pdf_file_name ?? "PDF"}
          </Button>
        )}
      </div>
    </div>
    ...
    <div className="rounded-lg border bg-card p-5 space-y-3">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {lines.map((l) => (
              <LineRow key={l.id} line={l} onDone={onDone} />
            ))}
            {lines.length === 0 && (
              <tr>
                <td colSpan={9} className="p-6 text-center text-muted-foreground">
                  No line items. Add one to start review.
                </td>
              </tr>
            )}
          </tbody>
...
function LineRow({ line, onDone }: { line: any; onDone: () => void }) {
  ...
  const updateReview = async (review_status: string) => {
    const { error } = await supabase
      .from("vendor_line_items")
      .update({ review_status: review_status as VendorLineReview })
      .eq("id", line.id);
  ...
  const markReviewed = async () => {
    ...
    const { error } = await supabase
      .from("vendor_line_items")
      .update({ review_status: "approved" satisfies VendorLineReview })
      .eq("id", line.id);
    setMarking(false);
    if (error) toast.error(error.message);
    else {
  ...
  return (
    <>
      <tr className="border-t hover:bg-muted/30">
        ...
        <td className="p-2">
          ...
          {typeof line.extraction_confidence === "number" && (
            <div className="text-[10px] text-muted-foreground">
              AI confidence: {Math.round(line.extraction_confidence * 100)}%
            </div>
          )}
          {line.extraction_warning && (
            <div className="text-xs text-amber-700">⚠ {line.extraction_warning}</div>
          )}
        </td>
        <td className="p-2">
          {line.quantity}{" "}
        ...
        <td className="p-2">
          {fmtMoney(line.suggested_retail_price ?? suggestRetail(line.wholesale_cost))}
        </td>
        ...
        <td className="p-2">
          <div className="flex items-center gap-1.5">
            <Select value={line.review_status} onValueChange={updateReview}>
              ...
              <SelectContent>
                {VENDOR_LINE_REVIEW.map((s) => (
                  <SelectItem key={s} value={s}>
                    {VENDOR_LINE_REVIEW_LABELS[s]}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        <td className="p-2">
          <OpsBadge
            label={
              VENDOR_LINE_PRICING_LABELS[
                line.pricing_status as keyof typeof VENDOR_LINE_PRICING_LABELS
              ]
            }
            tone={pricingTone(line.pricing_status)}
          />
        </td>
        <td className="p-2">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Line #">
          <Input
            ...
            value={f.line_number ?? ""}
            onChange={(e) =>
              setF({ ...f, line_number: e.target.value === "" ? null : Number(e.target.value) })
            }
          />
        </Field>
      </div>
      <Field label="Clean item name">
        <Input
          value={f.clean_item_name ?? ""}
          onChange={(e) => setF({ ...f, clean_item_name: e.target.value })}
        />
      </Field>
      <Field label="Raw description">
        <Textarea
          rows={2}
          value={f.raw_description ?? ""}
          onChange={(e) => setF({ ...f, raw_description: e.target.value })}
        />
      </Field>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Item type">
          <Select
            value={f.item_type ?? "_unset"}
            onValueChange={(v) => setF({ ...f, item_type: v === "_unset" ? null : v })}
          >
            <SelectTrigger>
        ...
        <Field label="Scientific name">
          <Input
            ...
            onChange={(e) => setF({ ...f, scientific_name: e.target.value })}
          />
        </Field>
        <Field label="Vendor item ID">
          <Input
            value={f.vendor_item_id ?? ""}
            onChange={(e) => setF({ ...f, vendor_item_id: e.target.value })}
          />
        </Field>
        ...
        <Field label="Origin / region">
          <Input
            value={f.origin_region ?? ""}
            onChange={(e) => setF({ ...f, origin_region: e.target.value })}
          />
        </Field>
        <Field label="Size">
        ...
        <Field label="Quantity">
          <Input
            ...
            value={f.quantity ?? ""}
            onChange={(e) =>
              setF({ ...f, quantity: e.target.value === "" ? null : Number(e.target.value) })
            }
          />
        </Field>
        <Field label="Wholesale cost">
          <Input
            type="number"
            step="0.01"
            value={f.wholesale_cost ?? ""}
            onChange={(e) =>
              setF({ ...f, wholesale_cost: e.target.value === "" ? null : Number(e.target.value) })
            }
          />
        </Field>
        <Field label="Vendor sell price">
          <Input
            type="number"
            step="0.01"
            value={f.vendor_sell_price ?? ""}
            onChange={(e) =>
              setF({
                ...f,
                vendor_sell_price: e.target.value === "" ? null : Number(e.target.value),
              })
            }
          />
        </Field>
        <Field label="Regular price">
          <Input
            ...
            onChange={(e) =>
              setF({ ...f, regular_price: e.target.value === "" ? null : Number(e.target.value) })
            }
          />
        </Field>
        <Field label="Line total">
          <Input
            ...
            value={f.line_total ?? ""}
            onChange={(e) =>
              setF({ ...f, line_total: e.target.value === "" ? null : Number(e.target.value) })
            }
          />
        </Field>
        <Field label="Suggested retail">
          <Input
            type="number"
            step="0.01"
            value={f.suggested_retail_price ?? ""}
            onChange={(e) =>
              setF({
                ...f,
                suggested_retail_price: e.target.value === "" ? null : Number(e.target.value),
              })
            }
          />
        </Field>
      </div>
      <Field label="Extraction warning">
        <Input
          value={f.extraction_warning ?? ""}
          onChange={(e) => setF({ ...f, extraction_warning: e.target.value })}
        />
      </Field>
      <Field label="Notes">
        <Textarea
...
function ChargesSection({
  ...
  const submit = async () => {
    const { error } = await supabase
      .from("vendor_batch_charges")
      .insert({ ...f, vendor_batch_id: batchId });
    if (error) {
      toast.error(error.message);
      return;
    }
    ...
    setOpen(false);
    setF({ charge_type: "freight", amount: 0, quantity: 1 });
    onDone();
  };
    ...
    <div className="rounded-lg border bg-card p-5 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>New batch charge</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              <Field label="Type">
                <Select value={f.charge_type} onValueChange={(v) => setF({ ...f, charge_type: v })}>
                  ...
                  <SelectContent>
                    {VENDOR_CHARGE_TYPES.map((t) => (
                      <SelectItem key={t} value={t}>
                        {VENDOR_CHARGE_LABELS[t]}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
        ...
        <tbody>
          {charges.map((c) => (
            <tr key={c.id} className="border-t">
              <td className="p-2">
                {VENDOR_CHARGE_LABELS[c.charge_type as keyof typeof VENDOR_CHARGE_LABELS]}
              </td>
...
function ConvertButton({
  ...
  const eligible = lines.filter(
    (l) =>
      ...
      l.pricing_status === "approved" &&
      !l.converted_inventory_item_id,
  );
  const run = async () => {
    if (eligible.length === 0) {
      toast.info("No eligible lines (need approved review + approved pricing)");
      return;
    }
    setBusy(true);
    try {
  ...
  return (
    <Button onClick={run} disabled={busy}>
      {busy ? "Converting…" : `Convert ${eligible.length} to inventory`}
    </Button>
  );
}

function ExtractAiButton({
  batchId,
  hasPdf,
  extractionStatus,
  onDone,
    ...
    try {
      ...
      toast.success(
        `AI extracted ${res.lineCount} line(s), ${res.chargeCount} charge(s)` +
          (res.removedLines || res.removedCharges
            ? ` · replaced ${res.removedLines} AI line(s) / ${res.removedCharges} AI charge(s)`
            : "") +
  ...
  const label =
    extractionStatus === "ai_pending" ? "Extracting…" : busy ? "Working…" : "Extract with AI";
  ...
  return (
    <>
      <Button
        variant="secondary"
        disabled={disabled}
        onClick={() => setConfirmFirst(true)}
        title={!hasPdf ? "Upload a PDF first" : undefined}
      >
      ...
      <AlertDialog open={confirmFirst} onOpenChange={setConfirmFirst}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              AI will create draft line items and charges only. It cannot approve pricing, mark
              items reviewed, or convert anything to inventory. Staff review is required before any
              item becomes inventory. Human-entered header fields are never overwritten.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
      ...
      <AlertDialog open={confirmOverwrite} onOpenChange={setConfirmOverwrite}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This batch already has line items or charges. Re-extraction will only replace prior
              AI-created drafts. Human-created lines/charges and converted lines are preserved.
              Continue?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function exportTagsCsv(sellable: any[], drafts: Record<string, any>, getDraft: (l: any) => any) {
  ...
  for (const l of sellable) {
    ...
    if (qty <= 0) continue;
    const name =
      (d.clean_item_name ?? l.clean_item_name ?? l.raw_description ?? "").toString().trim() ||
      "Unnamed item";
    const priceNum =
      // Admin-approved price wins (it's what the item goes live at) so the printed
      // tag matches the live price; the override is only a fallback pre-approval.
      (l.approved_retail_price != null ? Number(l.approved_retail_price) : null) ??
      (d.override_retail_price != null && d.override_retail_price !== ""
        ? Number(d.override_retail_price)
        : null) ??
  ...
  if (rows.length === 0) {
    toast.error("No received quantities to export");
    return;
  }
...
function ReceiveSection({
  ...
  const { data: locs } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("store_locations").select("*").eq("is_active", true).order("name"))
        .data ?? [],
  });
  const { data: doaPhotos } = useQuery({
    queryKey: ["batch-doa-photos", batchId],
    queryFn: async () =>
      (await supabase.from("vendor_line_doa_photos").select("*").eq("vendor_batch_id", batchId))
        .data ?? [],
  });
  ...
  const getDraft = (l: any) =>
    drafts[l.id] ?? {
      received_quantity:
        l.received_quantity != null ? Number(l.received_quantity) : Number(l.quantity ?? 0),
  ...
  const zoneOf = (parentId: string | null) =>
    (locs ?? []).find((z: any) => z.id === parentId)?.name ?? "Unzoned";
    ...
    if (v === "dead_on_arrival") {
      ...
      if (missing.length > 0) {
        toast.warning("DOA requires 2 photos", {
          description:
            "Upload one photo of the animal still in the bag, and one laying on the Styrofoam lid. Required by the wholesaler.",
  ...
  const submit = async () => {
    if (sellable.length === 0) {
      toast.info("No sellable lines to receive");
      return;
    }
    ...
    <div className="rounded-lg border bg-card p-5 space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Every save is recorded in the receive audit trail with timestamp + user. DOA lines
            require an in-bag and on-lid photo per wholesaler policy.
          </p>
        </div>
        <div className="flex gap-2">
          <Button
            variant="outline"
            onClick={() => exportTagsCsv(sellable, drafts, getDraft)}
          ...
          <Button onClick={submit} disabled={busy || sellable.length === 0}>
            {busy ? "Saving…" : "Save received"}
          </Button>
        </div>
      </div>
      {sellable.length > 0 &&
        (() => {
          const totals = sellable.reduce(
            (acc, l) => {
              const d = getDraft(l);
          ...
          return (
            <div
              className={`flex flex-wrap items-center gap-3 rounded-md border px-3 py-2 text-xs ${matches ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-amber-300 bg-amber-50 text-amber-900"}`}
            >
              <span className="font-semibold">
                {matches
                  ? "✓ Matches PO"
                  : `⚠ ${totals.diffLines} line${totals.diffLines === 1 ? "" : "s"} differ from PO`}
              </span>
              <span>
                Ordered: <b>{totals.ordered}</b>
              </span>
              <span>
                Received: <b>{totals.received}</b>
              </span>
              <span>
                Lost: <b>{totals.lost}</b>
              </span>
            </div>
          );
        })()}
      {sellable.length === 0 && (
        <p className="text-sm text-muted-foreground p-4 text-center">
          No unconverted sellable lines. Add a draft line or run AI extraction first.
        </p>
      )}
      <div className="overflow-x-auto">
        {sellable.length > 0 && (
          <table className="w-full text-sm">
            ...
            <tbody>
              {sellable.map((l) => {
                ...
                const have = photoCountsByLine[l.id] ?? new Set();
                const doaActive =
                  d.loss_reason === "dead_on_arrival" && Number(d.lost_quantity ?? 0) > 0;
                const doaComplete = have.has("in_bag") && have.has("on_lid");
                return (
                  <tr
                    key={l.id}
                    id={`receive-row-${l.id}`}
                    className="border-t align-top scroll-mt-24 target:bg-amber-50"
                  >
                    <td className="p-2">
                      <div className="font-medium">
                        {l.clean_item_name || l.raw_description || "(no name)"}
                      </div>
                      {l.scientific_name && (
                      ...
                      {doaActive && (
                        <button
                          onClick={() => setDoaTarget(l)}
                          className={`mt-1 inline-flex items-center gap-1 text-[11px] rounded px-1.5 py-0.5 ${doaComplete ? "bg-emerald-100 text-emerald-800" : "bg-destructive/15 text-destructive"}`}
                        >
                          {doaComplete ? (
                    ...
                    <td className="p-2">
                      <Select
                        value={d.item_type ?? "_unset"}
                        onValueChange={(v) =>
                          setDraft(l.id, { item_type: v === "_unset" ? null : v })
                        }
                      >
                    ...
                    <td className="p-2 text-muted-foreground">
                      {l.quantity ?? "—"} {l.size && <span className="text-xs">{l.size}</span>}
                    </td>
                    <td
                      className={`p-2 ${Number(d.received_quantity ?? 0) !== Number(l.quantity ?? 0) ? "bg-amber-50" : ""}`}
                    >
                      <Input
                        className="h-8 w-20"
                        type="number"
                        ...
                        onChange={(e) =>
                          setDraft(l.id, {
                            received_quantity: e.target.value === "" ? 0 : Number(e.target.value),
                          })
                        }
                      />
                    ...
                    <td className="p-2">
                      <Input
                        ...
                        type="number"
                        step="0.01"
                        value={d.lost_quantity ?? 0}
                        onChange={(e) =>
                          setDraft(l.id, {
                            lost_quantity: e.target.value === "" ? 0 : Number(e.target.value),
                          })
                        }
                      />
                    </td>
                    <td className="p-2">
                      <Select
                        value={d.loss_reason ?? "_none"}
                        onValueChange={(v) => onLossReasonChange(l, v === "_none" ? null : v)}
                      >
                    ...
                    <td className="p-2">
                      ...
                      {l.approved_retail_price != null && (
                        <div className="text-[10px] text-muted-foreground mt-0.5">
                          Approved: {fmtMoney(l.approved_retail_price)}
                        </div>
                      )}
                    </td>
                    <td className="p-2">
                      <Select
                        value={d.assigned_location_id ?? "_none"}
                        onValueChange={(v) =>
                          setDraft(l.id, { assigned_location_id: v === "_none" ? null : v })
                        }
                      >
                        ...
                        <SelectContent>
                          ...
                          {tanks.map((t: any) => (
                            <SelectItem key={t.id} value={t.id}>
                              {zoneOf(t.parent_location_id)} · {t.name}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
      ...
      <p className="text-xs text-muted-foreground">
        Suggested retail = 3 × wholesale cost. Override is captured per line; admin still approves
        final retail in Pricing Approval before conversion.
      </p>
      ...
      {doaTarget && (
        <DoaPhotoDialog
          ...
          onClose={() => {
            setDoaTarget(null);
            qc.invalidateQueries({ queryKey: ["batch-doa-photos", batchId] });
          }}
        />
      )}
      {historyTarget && (
      ...
      <BarcodeScanDialog
        ...
        onMatch={(line, code) => {
          ...
          const next = scanAutoInc
            ? Number(d.received_quantity ?? 0) + 1
            : Number(d.received_quantity ?? 0);
          setDraft(line.id, { received_quantity: next });
          toast.success(`Scanned ${code}`, {
            description: `${line.clean_item_name ?? line.raw_description ?? "line"} · received now ${next}`,
            duration: 2000,
          });
          // Scroll into view
          setTimeout(() => {
...
function DoaPhotoDialog({
  ...
  const loadPreview = async (k: string) => {
    ...
    if (!p || urls[k]) return;
    try {
      const { url } = await getUrl({ data: { path: p.storage_path } });
      setUrls((u) => ({ ...u, [k]: url }));
    } catch {}
  };
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        ...
        <p className="text-sm text-muted-foreground">
          Wholesaler policy: every DOA must have two photos — still in the sealed bag, and laying on
          the Styrofoam lid. Both are required before this line can be saved.
        </p>
            ...
            return (
              <div key={k} className="border rounded-md p-3 space-y-2">
                ...
                <div className="aspect-square bg-muted/40 rounded flex items-center justify-center overflow-hidden">
                  {urls[k] ? (
                    <img src={urls[k]} alt={label} className="w-full h-full object-cover" />
                  ) : (
                ...
                <label className="block">
                  ...
                  >
                    <span>
                      {busyKind === k ? "Uploading…" : p ? "Replace" : "Capture / upload"}
                    </span>
                  </Button>
                </label>
              </div>
...
function ReceiveHistoryDialog({ line, onClose }: { line: any; onClose: () => void }) {
  const { data: logs } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("vendor_line_receive_logs")
          .select("*")
          .eq("vendor_line_item_id", line.id)
          .order("created_at", { ascending: false })
      ).data ?? [],
  });
  const actorIds = Array.from(new Set((logs ?? []).map((l: any) => l.actor_id).filter(Boolean)));
  const locIds = Array.from(
    new Set((logs ?? []).map((l: any) => l.assigned_location_id).filter(Boolean)),
  );
  const { data: profiles } = useQuery({
    queryKey: ["receive-log-profiles", actorIds.join(",")],
    enabled: actorIds.length > 0,
    queryFn: async () =>
      (await supabase.from("profiles").select("id,display_name,email").in("id", actorIds)).data ??
      [],
  });
  const { data: locsMeta } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("store_locations").select("id,name").in("id", locIds)).data ?? [],
  });
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-auto">
        <DialogHeader>
          <DialogTitle>
            Receive history — {line.clean_item_name || line.raw_description || "line"}
          </DialogTitle>
        </DialogHeader>
        {(!logs || logs.length === 0) && (
          <p className="text-sm text-muted-foreground">No receive actions yet.</p>
        )}
        <div className="space-y-2">
          {(logs ?? []).map((g: any) => {
            ...
            if (g.received_quantity !== g.prev_received_quantity)
              changes.push(
                `received ${g.prev_received_quantity ?? "—"} → ${g.received_quantity ?? "—"}`,
              );
            if (g.lost_quantity !== g.prev_lost_quantity)
              changes.push(`lost ${g.prev_lost_quantity ?? "—"} → ${g.lost_quantity ?? "—"}`);
            if ((g.loss_reason ?? null) !== (g.prev_loss_reason ?? null))
              changes.push(`reason ${g.prev_loss_reason ?? "—"} → ${g.loss_reason ?? "—"}`);
            ...
            if (Number(g.override_retail_price ?? 0) !== Number(g.prev_override_retail_price ?? 0))
              changes.push(
                `override retail ${fmtMoney(g.prev_override_retail_price)} → ${fmtMoney(g.override_retail_price)}`,
              );
            return (
              <div key={g.id} className="rounded border p-2 text-xs">
                ...
                <div className="mt-1 text-muted-foreground">
                  {changes.length ? changes.join(" · ") : "no field changes (re-save)"}
                </div>
