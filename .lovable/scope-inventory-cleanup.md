...
15. **[DB+App] ✅ RESOLVED (`rack_position`) — promoted to a real column, fully cut over.**
    Lovable shipped the `rack_position text` column + `(location_id, rack_position)` index + backfill;
    app cut over to column-only reads/writes (PR #59 + #60); Lovable then dropped the `attrs.rack_position`
    key from existing rows (migration `20260618184654`). Done end-to-end — no app path touches
    `attrs.rack_position`. `stock_mode`/`inventory_role` stay in attrs; `ocr_text`/`ocr_extracted_at` not
    dropped; `attrs.clover_item_id` kept by design (orphan-recovery, documented). Handoff:
    `handoff-attrs-to-columns.md`. Lovable shipped the `rack_position text` column + `(location_id, rack_position)` index and backfilled it. App cutover (this PR): all **list/query reads** now use the `rack_position` column (inventory list, Pricing Queue coral row, Coral Discovery overview); **writes dual-write** the column — `buildInventoryInsert` mirrors `attrs.rack_position`→column on insert, and `updateInventoryAttrs` mirrors it on edit (uppercased). Next: ping Lovable that reads are on the column so they drop the attrs key; then a follow-up moves the detail attrs-editor field to the column and stops dual-writing. **`stock_mode`/`inventory_role` stay in attrs** (Lovable confirmed). **`ocr_text`/`ocr_extracted_at` NOT dropped** (confirmed). **`attrs.clover_item_id`: deferred — see note.** Decision/handoff: `handoff-attrs-to-columns.md`.
