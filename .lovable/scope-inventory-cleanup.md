...
15. **[DB+App] ⏳ HANDED TO LOVABLE (scope/decision) — Promote load-bearing `attrs` keys to real columns**: `rack_position`, `stock_mode`, `inventory_role` + drop duplicate `attrs.clover_item_id` in favor of `clover_item_links`. Framed as a proposal because it partly conflicts with `design-coral-stock-tracking.md` (which deliberately keeps `stock_mode` in attrs). Recommendation: promote `rack_position` (column + index); leave `stock_mode` in attrs; `inventory_role` optional; `clover_item_id` = backfill + app cutover. See `handoff-attrs-to-columns.md`.
